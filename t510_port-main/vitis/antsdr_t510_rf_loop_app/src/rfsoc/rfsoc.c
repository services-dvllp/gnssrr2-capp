...
u32 XRFdc_SetQMC(XRFdc *RFdcInstPtr, u32 Type) {
	u32 Status;
		...
		for (int i = 0; i < 4; i++)
			for (int j = 0; j < 2; j++)
				Status = XRFdc_UpdateEvent(RFdcInstPtr, Type, i, j * (Type + 1),
				XRFDC_EVENT_QMC);
		gpio_set_value(LMK_SYNC, 1);
		usleep(1000);
		gpio_set_value(LMK_SYNC, 0);
...
//延时等待时间稳定
	...
	while (dac_sync != 0) {
		...
		DAC_Sync_Config.Target_Latency = -1;//不确定Latency
  ...
  int	adc_sync = -1;
	while (adc_sync != 0) {
		...
		printf("\n\r=== Run ADC Sync ===\n\r");
		/* Initialize ADC MTS Settings */ //adc tile0为基准
	...
	/****************************************************************************/
	for (Tile_DAC = 0; Tile_DAC < 4; Tile_DAC++) {
		for (Pair_DAC = 0; Pair_DAC < 2; Pair_DAC++) {
			...
			//DAC特有最后一个参数是mode，1 (First Nyquist Zone),2(Second Nyquist Zone)
	...
	/****************************************************************************/
	for (Tile_ADC = 0; Tile_ADC < 4; Tile_ADC++) {
	...
	/****************************************************************************/
	//将DAC和ADC的混频器设置都通过XRFdc_UpdateEvent更新上去
	/****************************************************************************/
	for (Tile_DAC = 0; Tile_DAC < 4; Tile_DAC++) {
		for (Pair_DAC = 0; Pair_DAC < 2; Pair_DAC++) {
