 ...
 * rfdc_init.c
 *
 * Callable RFDC initialisation for the gnssrr2-stream RX/TX apps.
 *
 * This is the Linux userspace ANTSDR T510 CH8 RF initialisation sequence,
 * vendored from linux_app/main_linux.c (which builds the standalone
 * `t510_rf_init` tool). The ONLY change from main_linux.c is that the
 * original `int main(int argc, char *argv[])` plus its getopt argument
 * parsing has been replaced by a callable
 * `int rfdc_init(int num_tiles_arg, int skip_mts_arg)`, so the same,
 * otherwise-unmodified bring-up sequence can be invoked from inside
 * rx_stream and tx_stream at startup instead of being run as a separate
 * process. Every RFDC/LMK/tile/NCO/QMC/MTS step below is identical to
 * main_linux.c.
 *
 ...
 * rfdc_init
 *
 * Programmatic entry point replacing main_linux.c's main(). The CLI parsing
 * (-t / --skip-mts) is gone; the equivalent values are passed by the caller:
 *   num_tiles_arg : tiles to configure (clamped to 1-4; out-of-range -> 1,
 *                   matching the original default and -t validation)
 *   skip_mts_arg  : non-zero skips the Multi-Tile Synchronisation loop,
 *                   exactly like the original --skip-mts flag.
 * The body below is otherwise identical to main_linux.c.
 * -------------------------------------------------------------------------- */
int rfdc_init(int num_tiles_arg, int skip_mts_arg)
{
    ...
    /* Equivalent of the original -t / --skip-mts arguments, now passed in by
     * the caller. Clamp num_tiles to the original 1-4 range (the original
     * -t parser rejected anything else; here we fall back to the default 1
     * rather than abort, since this is called inside a streaming app). */
    uint32_t num_tiles = (num_tiles_arg >= 1 && num_tiles_arg <= 4)
                             ? (uint32_t)num_tiles_arg : 1U;
    int skip_mts = skip_mts_arg ? 1 : 0;
