...
#define PERIOD_SAMPLES       (T510_DMA_PERIOD_BYTES / sizeof(int16_t))
#define PERIODS_IN_BUFFER    (T510_DMA_BUFFER_BYTES / T510_DMA_PERIOD_BYTES)
...
static void format_tx_buffer_exact(int16_t *tx_buf)
{
    ...
    while ((src_idx + 8) <= UNIQUE_SAMPLES) {
...
int main(int argc, char **argv)
{
    ...
    printf("DMA capture: %u ms of signal = %llu bytes (%llu IQ frames, %llu DMA periods)\n",
           capture_ms,
           (unsigned long long)target_bytes,
           (unsigned long long)(target_bytes / T510_FRAME_BYTES),
           (unsigned long long)(target_bytes / T510_DMA_PERIOD_BYTES));
    clock_gettime(CLOCK_MONOTONIC, &t_start);
    t_last_period = t_start;

    /*
     * Continuously drain the cyclic RX ring until exactly capture_ms worth
     * of samples (at the design's 983,040 bytes/ms) has been written out.
     * The driver read() is a non-blocking snapshot of rx_buf, so we poll
     * the driver's rx_periods counter and copy out each period as soon as
     * its completion callback has fired.
     */
    while (bytes_written < target_bytes) {
        size_t chunk = T510_DMA_PERIOD_BYTES;
        off_t off;
        ...
        if (status.rx_periods == periods_consumed) {
            if (elapsed_ms_since(&t_last_period) > STALL_TIMEOUT_MS) {
                fprintf(stderr, "DMA stalled: no new RX period for %.0f ms\n",
                        STALL_TIMEOUT_MS);
                goto fail_stop;
            }
            continue; /* no new DMA period yet, keep polling */
        }
        clock_gettime(CLOCK_MONOTONIC, &t_last_period);

        /*
         * If we fell a full ring behind, the oldest unread periods were
         * overwritten by the cyclic DMA; resume from the most recently
         * completed period and account for the loss.
         */
        if (status.rx_periods - periods_consumed >= PERIODS_IN_BUFFER) {
            periods_dropped += status.rx_periods - periods_consumed - 1;
            periods_consumed = status.rx_periods - 1;
        }

        off = (off_t)((periods_consumed % PERIODS_IN_BUFFER) *
                      T510_DMA_PERIOD_BYTES);
        if (lseek(fd, off, SEEK_SET) < 0) {
            perror("lseek(rx)");
            goto fail_stop;
        }
        if (read_full(fd, rx_buf, T510_DMA_PERIOD_BYTES) != 0) {
            perror("read(rx)");
            goto fail_stop;
        }
        periods_consumed++;
        ...
        if (periods_read == 1) {
            if (preview_pairs > (PERIOD_SAMPLES / 2U))
                preview_pairs = PERIOD_SAMPLES / 2U;
            print_preview(rx_buf, preview_pairs);
        }

        if ((uint64_t)chunk > target_bytes - bytes_written)
            chunk = (size_t)(target_bytes - bytes_written);

        if (csv_fp)
            csv_append(csv_fp, rx_buf, chunk / sizeof(int16_t));
        if (bin_fp && bin_append(bin_fp, rx_buf, chunk) != 0)
            goto fail_stop;
        bytes_written += chunk;
    }
