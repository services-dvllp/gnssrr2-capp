...
module iq_requant_tx (
    ...
    // lim32 is the 32-bit positive limit; comparisons use full 32-bit signed
    // arithmetic to avoid sign-extension errors with 16-bit lim_neg.
    function automatic [16:0] rcomp;
        input signed [15:0] x;
        input [4:0]         shift;
        input [13:0]        lim;
        reg signed [31:0] scaled;
        reg signed [31:0] lim32;
        reg        [15:0] lim_pos16;
        reg        [15:0] lim_neg16;
        begin
            scaled    = $signed({{16{x[15]}}, x}) <<< shift;
            lim32     = {18'd0, lim};              // 32-bit positive limit (e.g. 8192)
            lim_pos16 = {2'b00, lim};              // 16-bit: +LIMIT
            lim_neg16 = (~{2'b00, lim}) + 16'd1;  // 16-bit two's-complement: -LIMIT
            if (scaled > lim32)
                rcomp = {1'b1, lim_pos16};
            else if (scaled < -lim32)
                rcomp = {1'b1, lim_neg16};
            else
