...
module record_replay_ctrl_tx #(
    ...
    (* X_INTERFACE_INFO = "xilinx.com:signal:clock:1.0 aclk CLK",
       X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF S_AXI:M_AXIS_RELOAD:M_AXIS_CONFIG, ASSOCIATED_RESET aresetn, FREQ_HZ 30720000" *)
    input  wire        aclk,
...
NUM_READ_OUTSTANDING 1, NUM_WRITE_OUTSTANDING 1" *)
    ...
    // FIR coefficient-set activation -> fir_compiler_tx/S_AXIS_CONFIG (PG149:
    // reloaded coefficients only become ACTIVE after a config transaction)
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            if (s_axi_bvalid && s_axi_bready)
                s_axi_bvalid <= 1'b0;
            // DONE = coefficients ACTIVE (reload + config accepted); ERR = timeout
            if (fir_reload_done)
