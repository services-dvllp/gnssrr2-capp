...
module programmable_interp_tx (
    ...
    // Look-ahead ready: also assert during the last output beat of the current
    // buffer so the upstream can present the next beat in the same cycle.
    // This eliminates the 1-cycle loading bubble that would otherwise stretch
    // every 4-sample group to 5 RFDC clocks, shifting the effective TX rate
    // from the intended (Fs/M) to (Fs/M × M/(M+1)).
    wire last_beat;
    assign last_beat = buf_loaded &&
                       (({2'd0, s_idx} + {1'b0, mask_pop}) >= 4'd4) &&
                       (!out_valid_r || m_axis_tready);
    assign s_axis_tready = !buf_loaded || last_beat;
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            if (buf_loaded && (!out_valid_r || m_axis_tready)) begin
                ...
                // If the upstream already has the next beat ready (tready was
                // asserted via last_beat this same cycle), pre-load it now so
                // there is no idle cycle between consecutive input beats.
                if (({2'd0, s_idx} + {1'b0, mask_pop}) >= 4'd4) begin
                    s_idx <= 2'd0;
                    if (s_axis_tvalid) begin
                        samp_buf   <= s_axis_tdata;
                        buf_loaded <= 1'b1;   // stay loaded — no idle cycle
                        if (!cnt_clr) in_beat_count <= in_beat_count + 32'd1;
                    end else begin
                        buf_loaded <= 1'b0;
                    end
