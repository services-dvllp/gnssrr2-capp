...
module iq_bit_unpacker_tx (
    ...
    // Look-ahead ready: also assert during the final output beat of the current
    // buffer so the upstream can present the next beat in the same cycle.
    // Without this, a 1-cycle loading bubble appears between every pair of
    // consecutive input beats.  For dual 8-bit (step_bits=128, 2 beats/input)
    // the bubble creates a 2-valid / 1-idle pattern at 2/3 throughput; when
    // combined with programmable_interp_tx's preload the effective rate drops
    // to 50%, halving the observed TX frequency (e.g. 1.76 MHz → 0.88 MHz).
    wire last_beat;
    assign last_beat = buf_valid &&
                       ((rd_ptr + step_bits) >= 10'd256) &&
                       (!out_valid_r || m_axis_tready);
    assign s_axis_tready = (!buf_valid || last_beat) &&
                           !(out_valid_r && !m_axis_tready);
        ...
        if (!aresetn) begin
            ...
            if (buf_valid && (!out_valid_r || m_axis_tready)) begin
                ...
                if ((rd_ptr + step_bits) >= 10'd256) begin
                    // Last beat of current buffer.  If the upstream already has
                    // the next beat ready (tready was asserted via last_beat this
                    // same cycle), pre-load it now so there is no idle cycle
                    // between consecutive input beats.
                    if (s_axis_tvalid) begin
                        in_buf    <= s_axis_tdata;
                        rd_ptr    <= 9'd0;
                        buf_valid <= 1'b1;   // stay loaded — no bubble
                        if (!cnt_clr) in_beat_count <= in_beat_count + 32'd1;
                    end else begin
                        buf_valid <= 1'b0;
                        rd_ptr    <= 9'd0;
