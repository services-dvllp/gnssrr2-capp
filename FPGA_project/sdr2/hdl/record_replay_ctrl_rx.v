...
module record_replay_ctrl_rx #(
    ...
    (* X_INTERFACE_INFO = "xilinx.com:signal:clock:1.0 aclk CLK",
       X_INTERFACE_PARAMETER = "ASSOCIATED_BUSIF S_AXI:M_AXIS_RELOAD:M_AXIS_CONFIG, ASSOCIATED_RESET aresetn, FREQ_HZ 15360000" *)
    input  wire        aclk,
...
NUM_READ_OUTSTANDING 1, NUM_WRITE_OUTSTANDING 1" *)
    ...
    // FIR coefficient-set activation AXI-Stream master -> fir_compiler_rx/S_AXIS_CONFIG
    // PG149: reloaded coefficients only become ACTIVE after a config transaction.
    // TDATA = 8 bits, value 0 = coefficient set 0 (single-set filter).
    (* X_INTERFACE_INFO = "xilinx.com:interface:axis:1.0 M_AXIS_CONFIG TDATA"  *) output wire [7:0]  m_axis_config_tdata,
    (* X_INTERFACE_INFO = "xilinx.com:interface:axis:1.0 M_AXIS_CONFIG TVALID" *) output wire        m_axis_config_tvalid,
    (* X_INTERFACE_INFO = "xilinx.com:interface:axis:1.0 M_AXIS_CONFIG TREADY" *) input  wire        m_axis_config_tready,
    ...
    always @(posedge aclk) begin
        if (!aresetn) begin
            ...
            // Latch reload_done into FIR_COEFF_CTRL[1] (DONE = coefficients ACTIVE,
            // i.e. reload streamed AND config transaction accepted by the FIR).
            // Latch reload_err  into FIR_COEFF_CTRL[2] (watchdog timeout).
            if (fir_reload_done)
                r_fir_coeff_ctrl[1] <= 1'b1;
            if (fir_reload_err)
                r_fir_coeff_ctrl[2] <= 1'b1;
        end
    end
